...
def process(data):
    try:
        int_data = struct.unpack('<' + 'i' * RX_CHANNELS, data)
        float_data = np.array([float(x) for x in int_data])
        if len(float_data) == RX_CHANNELS:
            if min(float_data) < -1000:
                print(float_data)
            return float_data
    except ValueError:
        pass
...
@prt.transformer(buffer=prt.FixedBuffer(buffer_size=1, shape=(RX_CHANNELS*TX_CHANNELS,), axis=0, use_np=True))
def analyze(data):
    ...
    for i in range(data.shape[0]):
        current_data = data[i]  # np.clip(data[i], -1000, 2**16)
        ...
        if is_on:
            ...
            drops = np.sum((current_data - np.array(last_peaks[tx_channel * RX_CHANNELS:(tx_channel+1) * RX_CHANNELS]))) / np.sum(np.array(last_peaks[tx_channel * RX_CHANNELS:(tx_channel+1) * RX_CHANNELS]) - np.array(troughs[tx_channel * RX_CHANNELS:(tx_channel+1) * RX_CHANNELS]))
            ...
            if (all_buffer_idx > MIN_ON and np.min(drops) < DROP_THRESH) or (auto_on and all_buffer_idx > 30):
                ...
                if all_buffer_idx < DATA_PER_CHANNEL:
                    all_buffer[all_buffer_idx, tx_channel, :] = data[i]
            ...
            rises = (current_data - np.array(troughs[tx_channel * RX_CHANNELS:(tx_channel+1) * RX_CHANNELS])) / (np.array(last_peaks[tx_channel * RX_CHANNELS:(tx_channel+1) * RX_CHANNELS]) - np.array(troughs[tx_channel * RX_CHANNELS:(tx_channel+1) * RX_CHANNELS]))
...
def main():
    ...
    serial_buffer = prt.FixedBuffer(BUFFER_SIZE, use_np=True, shape=(RX_CHANNELS,))
    data = prt.ByteSerialReadLayer.from_port(serial=serial_port, decoder=process, print_fps=False, preamble=b'UW',
                                             num_bytes=4 * RX_CHANNELS, buffer=serial_buffer)
    channel_data = analyze(data, print_fps=True)
    #
    # filtered = prt.ExponentialFilter(channel_data, alpha=.2)
    #
    prt.TimePlotLayer(data, window_size=600, n_channels=RX_CHANNELS, ylim=(0, 100000), lw=1)
    prt.TimePlotLayer(channel_data, window_size=600, n_channels=RX_CHANNELS * TX_CHANNELS, ylim=(0, 100000), lw=1)
    # #
