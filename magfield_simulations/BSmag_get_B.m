...
for nF = 1:BSmag.Nfilament % Loop on each filament
    ...
    parfor m = 1:size(X,1)
        bx = zeros(size(X,2),size(X,3));
        by = zeros(size(X,2),size(X,3));
        bz = zeros(size(X,2),size(X,3));
        for n = 1:size(X,2)
            for p = 1:size(X,3)
                ...
                % Sum
                bx(n,p) = bx(n,p) + mu0*I/4/pi*sum(DBx);
                by(n,p) = by(n,p) + mu0*I/4/pi*sum(DBy);
                bz(n,p) = bz(n,p) + mu0*I/4/pi*sum(DBz);
