...
state_count = 0
off_data = np.zeros((RX_CHANNELS, 1)) + 1
...
def segment(current_data):
    global state, tx_channel, state_count, global_count, mean_buffer, mean_buffer_index, running_means, running_means_index, last_mean, off_data
    flush = False
    # print(global_count)

    mean_buffer[mean_buffer_index] = np.linalg.norm(np.abs(current_data))# np.sum(np.abs(current_data))
    mean_buffer_index += 1
    ...
    if state == STATE_WAIT:
        ...
        for i in range(3):
            if abs(current_data[i]) < off_data[i]:
                off_data[i] = abs(current_data[i])
    ...
    if flush:
        ...
        off_data_copy = np.copy(off_data)
        off_data += 1
        return results.flatten(), state, off_data_copy
